...
- #112 daemon / runtime evaluation：若評估 companion daemon 或 runtime，必須先維持 core boundary；daemon / runtime 不得 retroactively redefine `spec-injector` as companion-first product。Current evaluation 見 [status-runtime-evaluation.md](status-runtime-evaluation.md)。
