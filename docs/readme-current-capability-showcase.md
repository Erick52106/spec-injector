...
| missing / unreadable / alias hint diagnostics | current with caveat | Missing and unreadable paths can be visible, with alias hints labeled as hints only. | `docs/source-trust.md`, `docs/workflow.md`, `docs/issue-to-context-pipeline.md` | confirmed-path guarantees, alias promoted to official reference |
...
| source trust / context budget design direction | Documents source trust categories and context-budget priorities for bounded task context. | `docs/source-trust.md`, `docs/product-moat.md` | design docs and policy boundaries | runtime scoring, guaranteed full-context coverage, or autonomous source selection | linked docs |
...
| minimal issue-to-context pipeline map | Shows the current issue-to-context flow and separates future lanes from implemented workflow. | `docs/issue-to-context-pipeline.md`, `docs/readme-showcase-readiness.md` | pipeline map and readiness checklist | full protocol runtime, companion runtime, remediation loop, or merged future lanes | README + linked docs |
