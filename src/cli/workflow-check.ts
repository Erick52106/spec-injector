...
const NEGATED_AUTONOMOUS_WORKER_ROUTING_SIGNAL_PATTERN = /\b(?:no|not|without)\s+autonomous worker[- ]routing\b|\bautonomous worker[- ]routing\s+(?:is\s+)?not\s+(?:requested|required)\b/i;
